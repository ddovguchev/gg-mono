...
const (
	...
	// 6 чанков ≈ 0.38с — пауза, после которой фраза уходит в Whisper.
	// Меньше 0.25с резало на обрывки внутри предложения.
	silenceChunksForEnd = 6

	// Минимальная длина фразы в чанках (~0.5с).
	minPhraseChunks = 8

	// Максимальная длина фразы (~6с). Если говорят без паузы —
	// принудительно режем, иначе транскрипт никогда не появится.
	// Короткий потолок резал длинные предложения на обрывки.
	maxPhraseChunks = 96
)
